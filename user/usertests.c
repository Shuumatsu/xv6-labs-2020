...
void exectest(char* s) {
    ...
    if (wait(&xstatus) != pid) { printf("%s: wait failed!\n", s); }
...
void forkforkfork(char* s) {
    ...
    if (pid == 0) {
        while (1) {
            ...
            if (fd >= 0) { exit(0); }
            if (fork() < 0) { close(open("stopforking", O_CREATE | O_RDWR)); }
        }
...
void sbrkbasic(char* s) {
    ...
    if (pid == 0) {
        ...
        for (b = a; b < a + TOOMUCH; b += 4096) { *b = 99; }
...
void sbrkfail(char* s) {
    ...
    if (pid == 0) {
        ...
        sbrk(10 * BIG);
        int n = 0;
        for (i = 0; i < 10 * BIG; i += PGSIZE) { n += *(a + i); }
